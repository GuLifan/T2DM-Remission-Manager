...
# 4) 跑一遍质量基线，确认环境正常
cd backend && uv run pytest -q -p no:warnings    # 期望 156 passed
cd frontend && npm test                          # 期望 17 passed / 7 files
...
| 项 | 命令 | 期望 |
| --- | --- | --- |
| 后端测试 | `cd backend; uv run pytest -q -p no:warnings` | **156 passed** |
| 后端静态检查 | `uv run ruff check .` | All checks passed |
| 前端测试 | `cd frontend; npm test` | **17 passed / 7 files** |
...
```
develop  [origin/develop: ahead 23]   ← 比远程领先 23 个提交（未推送；以 `git branch -vv` 的实时哈希为准）
main     1fc7286  [origin/main]                ← 停在 M3 时代
```

- **23 个提交未推送**（含 M4、M5、第一/二批改造、端口调整、全部文档修订）。Lifan 明确说过"**暂时不推送 M5，需要修改的内容很多**"。
...
- 数据库：`data/runtime/etmms.db`（**已被 .gitignore 排除**；当前迁移头 `4f8e1c2d9a70`；删掉即自动重建并跑迁移）。
...
### 7.1 当前状态：第一、二批已完成；第三批约束已定稿、代码待批准

Lifan 已于 2026-09-24 批准第一、二批方案，两批约束、实现、迁移、自动化与浏览器走查均已完成。第三批 UX-55/59/60 的角色、归属、测量与排序约束也已确认，实施方案见 `_SPEC/08` 8.7；当前只完成文档，**尚未获准修改第三批业务代码或数据库**。
...
| 第三批（流程与权限） | 📝 UX-55 指标可更新、UX-59 列表搜索排序、UX-60 账号隔离 | 约束已定稿；实施方案见 `_SPEC/08` 8.7，待 Lifan 审阅批准代码阶段 |

每批的过门条件见 `_SPEC/08` 8.2；**交付前必做**见 8.3（关闭测试能力但保留正式管理员、核验测试接口 403、清理测试数据）。

### 7.3 第三批已锁定的边界

第三批 T3-01…T3-06 已由 Lifan 审定：正式 `admin` 角色与测试标记分离；普通医生只写当前归属患者；只有管理员可转移归属；阶段复评更新身高/体重但不建趋势表；列表默认到期优先并支持五个字段双向排序。权威细节见 `_SPEC/06` 第八节与 `_SPEC/07` 16.6，实施前必须先取得 Lifan 对 `_SPEC/08` 8.7 的代码阶段批准。

### 7.4 尚未开始的两个里程碑

- **M6 依据检索库**：三份 PDF（`_DEV/医学依据/`）已确认是**文本型可抽取**；需要建 FTS5 索引 + 侧边栏检索抽屉 + **结果必须显示出处（材料名 + 页码）**。缺的素材索引表是 `_DEV/依据索引表_待填写_v1.0.csv`（36 行，当前 0 行已填）。
