...
### 7.1 当前状态：第一、二批已完成，无文档阻塞

Lifan 已于 2026-09-24 批准第一、二批方案。两批约束、实现、迁移、自动化与浏览器走查均已完成；下一步应按 `_SPEC/08` 进入第三批，开始前仍需继续遵守“先确认约束，再执行”的协作规则。
...
每批的过门条件见 `_SPEC/08` 8.2；**交付前必做**见 8.3（阻断测试账号、关闭测试模式、清理测试数据）。

### 7.3 第三批开始前需要 Lifan 核对的事

第二批 R2-01…R2-10 与 R2-12 已裁决并落地。进入第三批前重点复核 UX-55 指标更新、UX-59 搜索排序、UX-60 账号隔离/接管与 admin 改归属的权限边界；仍以 `_SPEC/06` 最新状态为准，**不要替 Lifan 决定**。
...
1. **阻断测试账号 `admin`**（`is_active=0`）并**关闭测试模式**；
