...
| M7 打包与交付 | ⏸ 已暂停 | `_SPEC/12` 保留既有方案；按 Lifan 指示停止打包与分发，等待重新授权 |
...
### 7.1 当前状态：M6-A/B/C/D 已完成，M7 打包与分发已暂停

Lifan 已批准 `_SPEC/11` 的五项 M6 决议，并依次授权 M6-A/B/C/D。M6-A 已完成迁移 `a1c9e7f2d4b8`、四来源导入与 684 个分块；M6-B 已完成状态/检索接口、登录鉴权、完整性门禁和隐私审计；M6-C 已把 `EvidenceDrawer` 接到真实接口；M6-D 已生成 36/36 条可追溯候选并增加复核状态门禁。全部候选仍为“待 Lifan/医学负责人复核”，不是已确认医学依据。后端 176 项、前端 30 项、文档 4/4 及隔离副本真实浏览器 Gate 均通过。`_SPEC/12` 只保留已完成的约束核对与实施方案；M7 代码、打包、分发和交付物均未启动，并按 2026-09-25 Lifan 指示暂停，直至重新明确授权。
...
- **M7 打包与交付**：已按 Lifan 指示暂停，等待重新明确授权。`_SPEC/12` 保留十项待审批决议；`scripts/build.py` 已写好但**未跑过完整流程**，不应在暂停期间运行。现脚本还未显式打入 Alembic 资源，也没有托盘退出、正式文件日志或干净环境 Gate。PyInstaller 6.22.3 + Python 3.14.4 **已完成早期兼容性烟测**；交付物尚未生成。
...
| M7 便携版打包 | ⏸ 已暂停；`_SPEC/12` 方案保留，等待 Lifan 重新授权；脚本未跑完整流程 |
