...
> **版本**：v1.3｜**日期**：2026-09-24｜**状态**：有效（第二轮第三批约束已并入，待实现）
...
:root {
  ...
  --etmms-content-max:    1120px; /* 内容区最大宽度：兼顾门诊 1080p 与开发者 2K */
...
| 数值输入 | 只接受数字；必须启用 `.num` 等宽数字；单位以固定、不可编辑的后缀显示在输入框右侧；范围或精度异常就地提示 |
| 日期输入 | 使用文本输入并统一显示 `yyyy/mm/dd`；允许连续输入 8 位数字后自动格式化；提交前校验真实日历日期，接口仍传 ISO `yyyy-mm-dd` |
| 年月输入 | 年、月使用两个独立数字框；出生年为 4 位、出生月为 1–12；病程年为 0–60、月为 0–11且默认 0 |
| 复选框 / 单选 | 18px，选中态用 `--etmms-primary`；**必须有关联标签**，点击标签即可切换 |
| 多选组 | 少量选项纵向排列；超过 6 项使用可展开多选控件，已选项以胶囊展示，并提供“其他（可填写）” |
| 长下拉 | 超过 10 项时必须使用分组或可检索/可滚动呈现；不得把 69 个科室平铺为失控长页面 |
...
- 患者列表标题区固定提供“批量导入”；导入结果必须显示成功、跳过、失败数量及逐行原因，任何一行都不得静默丢弃。
- 患者列表标题旁显示总人数；顶部固定提供姓名/住院号即时搜索，以及“批量导入”“新建患者”入口。
- 患者列表默认按“已逾期 → 今日到期 → 尚未到期 → 无复评日期”排列，同组内按最后编辑时间倒序；到期判断使用当前账号的**有效今天**。
- 姓名、住院号、最后编辑时间、下次复评日期、当前环节均可点击表头切换升序/降序，并必须显示当前排序方向；医生主动选择表头排序后，以该字段为主序，不再强制到期优先。
- 今日到期行使用 `--etmms-success-container` 浅绿色提示，已逾期行使用 `--etmms-warning-container` 浅橙色提示；手工排序时仍保留颜色标记。
- 所有账号可看全部患者；非本人患者在列表显示“只读”与责任医生，不得仅靠禁用色表达权限差异。
- 卡片不得嵌套超过两层。
...
| 导入结果 | 先显示“成功 N / 跳过 M / 失败 K”摘要，再逐行列出行号、住院号与原因；重复住院号属于跳过，不得覆盖既有患者 |
| 只读患者 | 允许查看患者档案与事件流水；工作台常驻显示“只读 · 责任医生：姓名”，所有会改变档案或流程的控件禁用；尝试操作时提示“您的账户暂无权限编辑此条记录” |
| 归属转移 | 只对管理员显示责任医生选择器；只能选择启用中的普通医生账号，提交前二次确认，成功后立即刷新责任医生与可编辑状态 |
...
- 底部固定 `FooterBar`：左侧开发者信息，中部项目名，右侧版本号与构建时间；不得承载临床信息。
...
- 内边距 24px；内容最大宽度 1120px；内容区使用 `margin: 0 auto` 水平居中，左侧导航保持固定位置。
...
| `NumericInput` | 纯数字输入 + 固定单位后缀 + 范围/精度提示 |
| `DateInput` | `yyyy/mm/dd` 文本日期输入、格式化与真实日期校验 |
| `YearMonthInput` | 出生年月与病程的双输入框 |
| `SelectWithOther` / `MultiSelectWithOther` | 结构化诊断依据与降糖药物选择，支持“其他”补充 |
| `ImportDialog` | xlsx/csv 文件选择与逐行导入结果 |
| `FooterBar` | 开发者、项目名、版本与构建时间，不承载临床内容 |
| `PatientDataTable` | 姓名/住院号搜索、表头排序、到期标记、总计数、责任医生与只读状态 |
| `PermissionNotice` | 非本人患者的常驻只读说明与统一无权限提示 |
| `OwnershipTransferDialog` | 管理员转移患者责任归属；显示原归属和新归属并要求二次确认 |
...
| v1.2 | 2026-09-24 | 并入第二轮第二批 UI-R1/R2/R3/R7/R8/R10：数值单位、文本日期、年月双框、结构化多选、导入反馈、底部状态栏及 1120px 居中内容区 | AI（据 Lifan 已审定裁决整理） |
| v1.3 | 2026-09-24 | 并入第三批 UI-R4/R5：患者搜索、到期优先与表头排序、今日/逾期标记、只读患者呈现、责任医生与管理员归属转移交互 | AI（据 Lifan 已审定裁决整理） |
