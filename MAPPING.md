...
> **版本**：v1.9｜**日期**：2026-09-25｜**状态**：M5 与第二轮三批完成；M6-A/B/C/D 已实施并通过工程验收，36 条候选待医学复核
...
## 十四、M6-D 规则依据候选映射（已实现）

| 能力 | 约束来源 | 实际实现位置 | 验证 |
| --- | --- | --- | --- |
| 36 条候选映射 | `_SPEC/11` M6-D03、M6-D | `_DEV/依据索引表_待填写_v1.0.csv`：依据性质、材料简称、页/章位置、关键结论 | 36/36 行必填字段完整；E3-B03/B04 按获批表结构合并一行 |
| 医学与软件边界 | 锁定稿十四、`_SPEC/07` 16.7 | 每行明确区分共识明确规则、共识倾向/原则、锁定稿/软件设计决定、非临床系统提示 | 不把软件分支强行归因于医学文献；原文摘录默认留空 |
| 医学复核状态 | `_SPEC/11` M6-D03.3、9.5 | 36 行统一为“待 Lifan/医学负责人复核” | `scripts/check_docs.py::check_evidence_index` 阻断缺行、缺字段、重复 ID 或未经批准的状态变化 |
| PDF 页码核对 | `_SPEC/11` M6-D02 | 2021 共识 PDF 第3页、2024 指南 PDF 第5—6/15页、2026 共识 PDF 第4/9页等候选位置 | 本地全文索引检索 + 物理 PDF 页渲染核对；不提交原 PDF 或渲染中间件 |
...
| v1.9 | 2026-09-25 | 回填 M6-D 36 条依据候选、医学/软件性质区分、统一待复核状态、PDF 物理页核对及自动门禁 |
